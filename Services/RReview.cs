...
{
    internal class RReview : IReview
    {
        public RReview() { }
        ...
        public async Task<Result<int>> Add(Review objOfTable)
        {
            ...
            try
            {
                ...
                {
                    cmd.CommandText = "INSERT INTO ОтзывКлиентаНаУслугу (НЗаявки, НКл, Оценка)" +
        ...
        public async Task<Result<List<Review>>> Get()
        {
            ...
            try
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    cmd.CommandText = "SELECT * FROM ОтзывКлиентаНаУслугу";
                    con.Open();
        ...
        public async Task<Result<int>> Remove(int НЗаявки, int НКл)
        {
            ...
            try
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    cmd.CommandText = "DELETE FROM ОтзывКлиентаНаУслугу WHERE НЗаявки =@val1 AND НКл = @val2";
        ...
        public async Task<Result<int>> Update(Review objOfTable, int НЗаявкиОлд, int НКлОлд)
        {
            ...
            try
            {
                ...
                using (var cmd = con.CreateCommand())
                {
                    cmd.CommandText = "UPDATE ОтзывКлиентаНаУслугу" +
